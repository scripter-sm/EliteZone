...
function EZ:LoadGUI()
	...
	run(function()
		...
		-- the segments inset by this so the track shows the split; their inner edges stay square and
		-- the rounded ends come from the track clipping them, so the gap reads even at any ratio
		local ratio_gap = 2
		local ratio_prefix = '<font color="#5ad16b">'
		...
		local function update_rivals(player)
			...
			ratio_value.Text = ratio_prefix..math.floor(targetinfo.dealt + 0.5)..ratio_split..math.floor(targetinfo.taken + 0.5)..ratio_suffix
