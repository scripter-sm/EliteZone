    ...
    if (fs.existsSync(library_path)) {
        for (const library of fs.readdirSync(library_path)) {
            if (library.endsWith('.lua')) {
                let data = fs.readFileSync(path.join(library_path, library), {encoding: 'utf8'});
                data = removeBOM(data);
                libraries.push({
    ...
    if (fs.existsSync(overlay_path)) {
        for (const overlay of fs.readdirSync(overlay_path)) {
            if (overlay.endsWith('.lua')) {
                let data = fs.readFileSync(path.join(overlay_path, overlay), {encoding: 'utf8'});
                data = removeBOM(data);
                overlays.push({
